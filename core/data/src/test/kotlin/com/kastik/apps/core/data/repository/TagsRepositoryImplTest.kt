...
class TagsRepositoryImplTest {
    ...
    @Test
    fun announcementTagsReturnsEmptyWhenNotSet() =
    ...
    @Test
    fun announcementTagsEmitsFromLocalSource() =
        runTest(testDispatcher) {
    ...
    @Test
    fun syncAnnouncementTagsFetchesRemoteTagsAndSavesToLocal() =
        runTest(testDispatcher) {
            // Enforce the use of baseTagDto for remote fetches using delegation
            val mockRemoteSource =
                object : TagsRemoteDataSource by tagsRemoteDataSource {
    ...
    @Test
    fun syncAnnouncementTagsRethrowsCancellationException() =
        runTest(testDispatcher) {
            val errorRepo =
                TagsRepositoryImpl(
                    crashlytics = fakeCrashlytics,
    ...
    @Test
    fun tagsReturnsEmptyWhenNotSet() =
        runTest(testDispatcher) {
    ...
    @Test
    fun syncSubscribableTagsFetchesRemoteTagsAndSavesToLocal() =
        runTest(testDispatcher) {
            val mockRemoteSource =
                object : TagsRemoteDataSource by tagsRemoteDataSource {
            ...
            assertThat(result).isNotEmpty()
            assertThat(result)
                .containsExactlyElementsIn(listOf(baseTagDto.toSubscribableTagProto().toTag()))
        }

    // --- Subscribed Tags Tests ---

    @Test
    fun subscribedTagsAreEmptyWhenNotSet() =
        runTest(testDispatcher) {
    ...
    @Test
    fun subscribedTagsReturnsSubscribedTagsWhenSet() =
        runTest(testDispatcher) {
    ...
    @Test
    fun subscribeToTagsReturnsSuccessResult() =
        runTest(testDispatcher) {
    ...
    @Test
    fun subscribeToTagsReturnsErrorOnException() =
        runTest(testDispatcher) {
            val errorRepo =
                TagsRepositoryImpl(
                    crashlytics = fakeCrashlytics,
